...
Session.prototype.disconnectSubscription = function(subscription) {
    if (!this.subscriptions[subscription.id]) return;

    this.subscriptions[subscription.id].disconnect();
...
Session.prototype.on = function(options, callback, status) {
    ...
    this.subscriptions[socket.id] = socket;
    return socket;
};
