...
Heartbeat.prototype.heartbeatReceivedCheck = function() {
    if (!this.heartbeatReceived) {
        this.session.log.error('heartbeat: hearbeat not received within timeout: restarting subscription socket.');
...
Heartbeat.prototype.start = function() {
    ...
    this.session.onMessage({ type: 'heartbeat' }, function(message) {
        self.heartbeatReceived = self.heartbeatReceived || (message.from === self.session.principal.id);
    });
