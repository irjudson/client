...
AuthRequest.afterRequest = function(session, err, resp, body, callback) {
    if (resp && resp.statusCode !== 200) err = body && body.error ? body.error : resp.statusCode;
    ...
    if (resp && resp.headers && resp.headers['x-n2-set-access-token']) {
        session.accessToken = JSON.parse(resp.headers['x-n2-set-access-token']);
        session.principal.accessToken = session.accessToken;

        // if we have a store, update the principal there.  (agents don't have a store)
        if (session.service.store) {
            session.service.store.set(session.principal.toStoreId(), session.principal.toStoreObject());
        }
    }
...
AuthRequest.get = function(session, options, callback) {
    ...
    return request.get(options, function(err, resp, body) {
        if (err) {
            session.log.error("authenticated request error: " + err);
            return callback(err);
        }
