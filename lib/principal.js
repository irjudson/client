...
Principal.prototype.accessTokenFor = function(session, options, callback) {
...
Principal.prototype.create = function(config, callback) {
    ...
    if (this.is('user') && (!this.email || !this.password || !this.name)) {
        ...
        if (!this.api_key && config.api_key) this.api_key = config.api_key;
        if (!this.api_key) return callback(new Error('Parameter api_key required for principal.create'));
