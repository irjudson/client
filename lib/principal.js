...
Principal.PUBLIC_KEY_BITS = 2048;
Principal.PUBLIC_KEY_EXPONENT = 65537;
...
Principal.prototype.authenticate = function(config, callback) {
    ...
    if (this.is('user')) {
        ...
        // we've used the nonce, so delete it.  we'll need to reconfig to get another if we have to reauth.
        delete config.nonce;
    }
...
/**
 * Creates and returns an access token for the passed principal using the session's security context.
 * Requires admin permission on principal.
 *
 ...
 * @param {Date} options Options desired for the created access token. In particular, 'expires' can be used to ask for a long lived access token.
 * @param {Function} callback Callback function of the form f(err, accessToken).
 **/

Principal.accessTokenFor = function(session, principalId, options, callback) {
    var endpoint = session.service.config.endpoints.principals + '/accesstoken';

    options.principal_id = principalId;

    session.post({ url: endpoint, json: options }, function(err, resp, body) {
...
Principal.prototype.create = function(config, callback) {
    ...
    if (this.is('user') && (!this.email || !this.password || !this.name)) {
        ...
        if (!this.api_key) return callback(new Error('Please pass api_key to create principal.'));

        var keys = ursa.generatePrivateKey(Principal.PUBLIC_KEY_BITS, Principal.PUBLIC_KEY_EXPONENT);

        self.public_key = keys.toPublicPem().toString('base64');
        self.private_key = keys.toPrivatePem().toString('base64');
    }

    var principalObject = JSON.parse(JSON.stringify(this));
    delete principalObject.private_key;

    request.post({ url: config.endpoints.principals, json: principalObject }, function(err, resp, body) {
        if (err) return callback(err);
        if (resp.statusCode != 200) return callback(body.error || resp.statusCode);
