...
CommandManager.prototype.execute = function() {
    ...
    if (!this.device) return console.log('CommandManager::execute: not in session of device, skipping execution.');
    if (this.executing) return console.log('CommandManager::execute: already executing command, skipping execute. current queue: ' + JSON.stringify(this.messageQueue));
    if (!this.messageQueue || this.messageQueue.length === 0) return console.log('CommandManager::execute: no commands in queue to execute, skipping execute.');

    var msToExecute = this.messageQueue[0].millisToTimestamp();
    if (msToExecute > 0) {
        console.log('CommandManager::execute: top message occurs in the future, setting timeout.');
    ...
    this.executeQueue(function(err) {
        ...
        if (err) console.log('CommandManager::execute: execution error: ' + err);
