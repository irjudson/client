...
CommandManager.prototype.execute = function() {
    ...
    if (!this.device) return this.session.log.debug('CommandManager::execute: not in session of device, skipping execution.');
    if (this.executing) return this.session.log.debug('CommandManager::execute: already executing command, skipping execute. current queue: ' + JSON.stringify(this.messageQueue));
    if (!this.messageQueue || this.messageQueue.length === 0) return this.session.log.info('CommandManager::execute: no commands in queue to execute, skipping execute.');

    this.session.log.info('CommandManager::execute: executing queue: ' + this.messageQueue.length + ': ' + JSON.stringify(this.messageQueue));

    var msToExecute = this.messageQueue[0].millisToTimestamp();
    if (msToExecute > 0) {
        this.session.log.info('CommandManager::execute: top message occurs in the future, setting timeout.');
    ...
    this.executeQueue(function(err) {
        ...
        if (err) this.session.log.info('CommandManager::execute: execution error: ' + err);
