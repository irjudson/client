...
CommandManager.prototype.filterToActiveCommands = function(callback) {
    async.filter(this.messageQueue, function(message, cb) {
        cb(message.millisToTimestamp() <= 0);
    }, callback)
};
...
CommandManager.prototype.start = function(session, messages, callback) {
    this.pause();
