...
CommandManager.prototype.execute = function() {
    ...
    if (this.paused) return console.log('execute: command execution paused, skipping execute.');
    if (this.executing) return console.log('execute: already executing command, skipping execute.');
...
CommandManager.prototype.start = function(session, filter, callback) {
    ...
    Message.find(session, filter, { sort: { ts: 1 } }, function(err, messages) {
        ...
        //console.log('starting command manager: ' + messages.length + ' existing messages.');

        self.messageQueue = messages;
        self.collapse();
        //console.log('after collapse we have ' + self.messageQueue.length + ' messages in queue.');

        self.resume();

        session.onMessage({ filter: filter }, function(message) {
