...
Message.remove = function(session, query, callback) {
    console.log("static remove: " + session);
    AuthRequest.remove(session, { url: session.service.config.messages_endpoint, query: query, json:true }, callback);
}

Message.prototype.remove = function(session, callback) {
    console.log("prototype remove: " + session);
