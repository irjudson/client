...
// TODO: there is an opportunity here to combine messages over the same session into one request.
//
// Challenges:
...
Message.sendQueue = function() {
    ...
    var context = Message.queuedMessages.slice(1);

    context.session.post({ url: session.service.config.endpoints.messages, json: context.messages }, function(err, resp, body) {
        if (resp.statusCode !== 200) {
            err = err || body.message || resp.statusCode;

            // bad request responses are fatal, otherwise requeue and try again.
            if (resp.statusCode !== 400)
                Message.queuedMessages.requeue(context);
        }

        // if we still have messages in the queue after all of this, set up the next process.
        if (Message.queuedMessages.length) Message.setupSendTimeout();
...
Message.setupSendTimeout = function() {
    if (!Message.sendTimeout) 
        Message.sendTimeout = setTimeout(function() { self.sendQueue(); }, 0);
};
