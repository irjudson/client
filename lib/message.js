...
Message.sendMany = function(session, messages, callback) {
    ...
    session.post({ url: session.service.config.messages_endpoint, json: messages }, function(err, resp, body) {
        if (err) return callback(err);
        ...
        if (callback) callback(null, sentMessages);
    });
};
...
Message.prototype.timestampString = function() {
