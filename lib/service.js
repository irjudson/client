...
Service.prototype.connect = function(principal, callback) {
    ...
    this.store.get(principal.toStoreId(), function(err, storedPrincipal) {
        if (storedPrincipal) {
            principal.updateAttributes(storedPrincipal);
            self.sessionWrapper(principal, principal.authenticate, callback);
        } else {
            self.sessionWrapper(principal, principal.create, callback);
        }
    });
};

Service.prototype.sessionWrapper = function(principal, authOperation, callback) {
    var self = this;
    this.authenticateSession(principal, authOperation, function(err, session, principal) {
        if (err) return callback(err);

        // TODO: need to have support for multiple auth failure handlers.        
        session.onAuthFailure(function() {
            session.stop();
            session.log.error('************ CLIENT WRAPPER CAUGHT AUTH FAILURE -- ATTEMPTING RESTART OF SESSION.');
